...
- **Rules stub vs live Gemini:** `.env.example` defaults to `TRIAGE_USE_STUB=0` and `DRAFT_USE_STUB=0` — add `GEMINI_API_KEY` and `run-overnight` hits Gemini Flash on `GEMINI_LIVE_ONLY_IDS` (logs show model backends). Set both stub flags to `1` for deterministic rules-only runs (matches `eval-urgency` without a key).
...
cp .env.example .env          # add GEMINI_API_KEY — default uses live Gemini on demo subset

# Optional rules-only: TRIAGE_USE_STUB=1, DRAFT_USE_STUB=1 (logs show rules-stub / rules-template)
# run-overnight prints backend names; live subset = GEMINI_LIVE_ONLY_IDS
